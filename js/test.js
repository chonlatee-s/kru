	...
	function showSCORE(data,score){
		...
		for(i=0;i<data.length;i++){
			...
			if(data[i].answer_user==0){
				...
				else show_ans_all += '<p class="float-right ref_text">&#10007;&nbsp;<del>อ้างอิง</del></p><hr>';
			}
			else{
				if(data[i].ref!="") show_ans_all += '<p class="float-right ref_text">&#10003;&nbsp;<a href="'+data[i].ref+'" target="_blank">อ้างอิง</a></p><hr>';
				else show_ans_all += '<p class="float-right ref_text">&#10003;&nbsp;<del>อ้างอิง</del></p><hr>';
		...
		show_ans_all +='<span style="font-size:14px; color:#7e6296">หมายเหตุ จะมีเฉลยเฉพาะข้อที่ตอบผิดเท่านั้น</span>';
...
// time
	...
	$('#btn_show_answer').click(function(event){
		...
		$('#show_ans_all').show();
		var targetOffset = $('#show_ans_all').offset().top;
